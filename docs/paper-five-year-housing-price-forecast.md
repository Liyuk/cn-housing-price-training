...
本文聚焦中国二手住宅市场，基于 70 城官方二手指数。数据与方法细节建立在作者此前的工作上（作者, 2026），本文不再重复数据构建过程，直接使用其产出的连续面板。
...
使用国家统计局 70 城二手住宅官方价格指数的月环比序列（2019-01 至 2026-06，90 个月，无缺口）。全国序列取 70 城月环比均值；北京取单城序列。数据构建细节见作者此前工作（作者, 2026）。
